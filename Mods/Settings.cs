...
exit 0";
        ...
        public static readonly Dictionary<ControllerBinding, List<KeyCode>> pcBindings = new Dictionary<ControllerBinding, List<KeyCode>>
        {
            { ControllerBinding.RightPrimaryButton, new List<KeyCode> { KeyCode.E } },
            { ControllerBinding.RightSecondaryButton, new List<KeyCode> { KeyCode.R } },
            { ControllerBinding.LeftPrimaryButton, new List<KeyCode> { KeyCode.F } },
            { ControllerBinding.LeftSecondaryButton, new List<KeyCode> { KeyCode.G } },
            { ControllerBinding.LeftGrip, new List<KeyCode> { KeyCode.LeftBracket } },
            { ControllerBinding.RightGrip, new List<KeyCode> { KeyCode.RightBracket } },
            { ControllerBinding.LeftTrigger, new List<KeyCode> { KeyCode.Minus, KeyCode.Equals } },
            { ControllerBinding.JoystickClick, new List<KeyCode> { KeyCode.Return } },
            { ControllerBinding.LeftOverride, new List<KeyCode> { KeyCode.LeftAlt } }
        };
        
        public static bool GetAnyKey(Settings.ControllerBinding binding)
        {
            if (!pcBindings.TryGetValue(binding, out var keys))
                return false;
        ...
        public static void LoadPCControls()
        {
            ...
            if (File.Exists(fileName))
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (Enum.TryParse(splitData[1], out ControllerBinding binding) &&
                        Enum.TryParse(splitData[0], out KeyCode key))
                        pcBindings[binding] = new List<KeyCode> { key };
                }
            }
            else
            {
